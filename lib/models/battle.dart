...
/// プレイヤーの位置情報スナップショット（battle ドキュメントの locations 配下）。
class PlayerLocation {
  final LatLng position;
  final DateTime updatedAt;

  const PlayerLocation({required this.position, required this.updatedAt});

  static PlayerLocation? fromMap(dynamic v) {
...
class Battle {
  ...
  /// 参加者の最新位置情報（uid → PlayerLocation）。
  /// 対戦中に一定間隔で各端末が自分の位置を書き込む。
  final Map<String, PlayerLocation> locations;
  ...
  /// 相手の最新位置情報を返す（未取得なら null）。
  PlayerLocation? oppLocation(String myUid) => locations[opponentOf(myUid)];

  /// 自分の最新位置情報を返す（未取得なら null）。
  PlayerLocation? myLocation(String myUid) => locations[myUid];
  ...
  factory Battle.fromMap(String id, Map<String, dynamic> map) {
    final locs = <String, PlayerLocation>{};
    final rawLocs = map['locations'];
    if (rawLocs is Map) {
      rawLocs.forEach((k, v) {
        final loc = PlayerLocation.fromMap(v);
        if (loc != null) locs[k.toString()] = loc;
      });
    }
    return Battle(
      id: id,
