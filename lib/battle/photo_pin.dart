...
class PhotoPin {
  ...
  /// colorPaletteBattle のインデックスリスト（画像判定色）
  ...
  Map<String, dynamic> toJson() => {
        'id': id,
        // Documents からの相対パスで保存する（絶対パスはコンテナ移動で壊れる）。
        'imagePath': AppPaths.toStorable(imagePath),
        'latitude': position.latitude,
        'longitude': position.longitude,
        'takenAt': takenAt.toIso8601String(),
        'colorNames': colorIds
            .where((i) => i >= 0 && i < colorNamesBattle.length)
            .map((i) => colorNamesBattle[i])
            .toList(),
  ...
  factory PhotoPin.fromJson(Map<String, dynamic> json) {
    ...
    if (json.containsKey('colorNames')) {
      final names = (json['colorNames'] as List<dynamic>)
          ...
          .toList();
      ids = names
          .map((name) => colorNamesBattle.indexOf(name))
          .where((i) => i >= 0)
          .toList();
    } else {
      ids = (json['colorIds'] as List<dynamic>? ?? [])
          .map((e) => (e as num).toInt())
          .where((i) => i >= 0 && i < colorPaletteBattle.length)
          .toList();
    }
