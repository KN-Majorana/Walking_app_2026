...
class _VersusLobbyScreenState extends State<VersusLobbyScreen> {
  ...
  Widget _withModeBar(Widget child) {
    // オーバーレイとして開かれている場合はモード切替バーが無いので、
    // 余白のバンドごと出さずに画面をそのまま使う。
    if (!BattleModeScope.isPresent(context)) return child;
    return ColoredBox(
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return _withModeBar(const Scaffold(
        body: Center(child: CircularProgressIndicator()),
      ));
    }
    final me = _me;
    if (me == null) {
      return _withModeBar(Scaffold(
