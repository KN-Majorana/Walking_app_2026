...
class _MapScreenState extends State<MapScreen> {
  ...
  /// 全履歴タイムラプスを再生中か（個別の散歩再生と区別する）。
  bool _isAllHistoryTimelapse = false;
  ...
  void initState() {
    ...
    // 起動直後、現在モードの初回チュートリアルを表示する。
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) TutorialService.showForMode(context, _mode);
    });
  }
  ...
  void _ensureLocationStream() {
    ...
    if (shouldRun) {
      ...
      if (_positionSub == null || _streamBackground != wantBackground) {
        ...
        _positionSub = LocationService.watchPositionRaw(
  ...
  void _onModeChanged(MapMode mode) {
    ...
    _ensureLocationStream();
    // 切り替え先モードの初回チュートリアルを表示する。
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) TutorialService.showForMode(context, mode);
    });
  }
  ...
  // ─── 全履歴タイムラプス ───
  /// これまでの全散歩記録を古い順につなげ、霧が一気に晴れていく様子を再生する。
  /// 個別の散歩再生（_startGhostPlayback）とは別で、軌跡カードは使わない。
  void _startAllHistoryTimelapse() {
    _stopGhostPlayback();
    final tracks = [..._savedTracks]
      ..sort((a, b) => a.startedAt.compareTo(b.startedAt));
    final allPoints = tracks
        .expand((track) => track.points.map((point) => point.position))
        .toList();
    if (allPoints.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('再生できる散歩記録がありません')));
      }
      return;
    }
    setState(() {
      _isAllHistoryTimelapse = true;
      _ghostClearedPoints.clear();
      _ghostPosition = allPoints.first;
    });
    _mapController.move(allPoints.first, 14.0);

    var index = 0;
    _ghostTimer = Timer.periodic(const Duration(milliseconds: 70), (timer) {
      if (!mounted) return;
      final end = (index + 8).clamp(0, allPoints.length).toInt();
      setState(() {
        _ghostClearedPoints.addAll(allPoints.sublist(index, end));
        _ghostPosition = allPoints[end - 1];
      });
      index = end;
      if (index >= allPoints.length) {
        // 最後に歴代の対戦で通った場所も晴らして締める。
        setState(() => _ghostClearedPoints.addAll(_battleFogPoints));
        timer.cancel();
        _ghostTimer = null;
      }
    });
  ...
  // ─── 再生モード時の「全履歴タイムラプス」ボタン ───
  Widget _buildAllHistoryTimelapseButton() {
    final playing = _isAllHistoryTimelapse;
    return Material(
      elevation: 6,
      borderRadius: BorderRadius.circular(16),
      color: playing ? const Color(0xFF2E7D32) : Colors.white,
      child: InkWell(
        borderRadius: BorderRadius.circular(16),
        onTap: _startAllHistoryTimelapse,
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              Icon(
                ...
                color: playing ? Colors.white : const Color(0xFF2E7D32),
              ),
              const SizedBox(width: 8),
              Text(
                playing ? '全履歴を再生中' : '全履歴タイムラプス',
                style: TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.bold,
                  color: playing ? Colors.white : const Color(0xFF2E7D32),
          ...
          FlutterMap(
            ...
            options: MapOptions(
              ...
              onPositionChanged: (camera, hasGesture) {
                final zoomChanged = (camera.zoom - _currentZoom).abs() >= 0.05;
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildAllHistoryTimelapseButton(),
                    ],
                  ),
                ),
              ),
